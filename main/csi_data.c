 ...
 */
void print_csi_data(const csi_data_t *csi_data) {
  const wifi_pkt_rx_ctrl_t *rx_ctrl = &csi_data->rx_ctrl;

  ets_printf("CSI_DATA,%d," MACSTR ",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
             csi_data->device_id, MAC2STR(csi_data->mac), rx_ctrl->rssi, rx_ctrl->rate,
             rx_ctrl->sig_mode, rx_ctrl->mcs, rx_ctrl->cwb, rx_ctrl->smoothing,
             rx_ctrl->not_sounding, rx_ctrl->aggregation, rx_ctrl->stbc, rx_ctrl->fec_coding,
             rx_ctrl->sgi, rx_ctrl->noise_floor, rx_ctrl->ampdu_cnt, rx_ctrl->channel,
...
void print_payload(const payload_t *payload) {
  for (int i = 0; i < payload->csi_data_arr_len; i++) {
    ets_printf("PAYLOAD,%d,%d,%d,", payload->device_id, payload->csi_data_arr_len, i);
    if (payload->device_id != 0 && i != 0) {
      // TODO: Investigate why the next elements are offset by 7 bytes
      print_csi_data((void *) &payload->csi_data_arr[i] + 7);
    } else {
      print_csi_data(&payload->csi_data_arr[i]);
    }
  }
}
...
void wifi_csi_rx_cb(void *ctx, wifi_csi_info_t *info) {
  ...
  // Overwrite the oldest CSI data if the array is full
  if (payload_index >= CSI_DATA_ARR_LEN)
    payload_index = 0;

  // Check if the received packet is from an ESP32 device
  ...
  ESP_LOGI(TAG, "Receiving CSI data from device %d", csi_data->device_id);
